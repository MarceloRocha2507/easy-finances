...
interface TotalAReceberCardProps {
  mesReferencia: Date;
  isLoading?: boolean;
}
...
export function TotalAReceberCard({ mesReferencia, isLoading: externalLoading }: TotalAReceberCardProps) {
  ...
  const isLoading = isLoadingTx || externalLoading;
  ...
  const totalGeral = receitasPendentes.reduce((s, t) => s + t.amount, 0);
    ...
    <>
      ...
      >
        <button className="w-full h-full text-left p-4" onClick={() => setOpen(true)}>
          ...
          <p className="text-muted-foreground text-xs sm:text-sm mb-1">A Receber</p>

          {isLoading ? (
            <Skeleton className="h-6 w-24 sm:h-7 sm:w-28 bg-muted/60" />
          ) : (
            <div className="space-y-0.5">
              <p className="text-lg sm:text-xl font-bold tabular-nums text-[#16A34A]">
        ...
        <DialogContent className="max-w-md overflow-hidden" noPadding>
          <div className="flex flex-col">
            ...
            <div className="px-6 pt-6 pb-4">
              ...
              <p style={{ color: "#16A34A", fontSize: 26, fontWeight: 700, lineHeight: 1.1 }} className="tabular-nums">
                +{formatCurrency(totalGeral)}
              </p>
              <div className="flex items-center gap-2 mt-3">
                {receitasPendentes.length > 0 && (
                  <span style={{ background: "#F3F4F6", color: "#374151", fontSize: 11, fontWeight: 500, borderRadius: 6, padding: "2px 8px" }}>
                    {receitasPendentes.length} {receitasPendentes.length === 1 ? "receita" : "receitas"}
                  </span>
                )}
                {receitasPendentes.length === 0 && (
            ...
            <div className="overflow-y-auto max-h-[55vh]">
              {receitasPendentes.length > 0 && (
                <div className="px-6 pt-4 pb-2">
                  <div className="flex items-center justify-between pb-2" style={{ borderBottom: "1px solid #F3F4F6" }}>
                    ...
                    <span style={{ color: "#16A34A", fontSize: 13, fontWeight: 700 }} className="tabular-nums">
                      +{formatCurrency(totalGeral)}
                    </span>
                  </div>
                  <div>
                    {receitasPendentes.map((t) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="tabular-nums" style={{ color: "#16A34A", fontSize: 14, fontWeight: 700, whiteSpace: "nowrap" }}>
              ...
              {receitasPendentes.length === 0 && (
                <div className="flex flex-col items-center justify-center py-10 text-center px-6">
                  ...
                  <p style={{ fontSize: 12, color: "#9CA3AF" }}>Nenhuma receita pendente neste mês.</p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between px-6 py-3" style={{ borderTop: "1px solid #E5E7EB" }}>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="tabular-nums" style={{ color: "#16A34A", fontSize: 14, fontWeight: 700 }}>
                  +{formatCurrency(totalGeral)}
                </span>
              </div>
            </div>
